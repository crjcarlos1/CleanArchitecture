package com.cralos.cleanarchitecture.framework.datasource.cache
...
class NoteDaoServiceTests : BaseTest() {
    ...
    init {
        ...
        insertTestData()
        noteDaoService =
            NoteDaoServiceImpl(noteDao = dao, noteMapper = cacheMapper, dateUtil = dateUtil)
    }
    ...
    //2. insert a new note, CBS
    @Test
    fun insertNewNote_CBS() = runBlocking {
    ...
    //6. delete new note, confirm deleted
    @Test
    fun insertNote_deleleteNote_confirmDeleted() = runBlocking {
        val newNote = noteDataFactory.createSingleNote(
            id = null,
            title = "super cool title",
    ...
    fun searchNotes_orderByDateASC_confirmOrder() = runBlocking {
        val noteList = noteDaoService.searchNotesOrderByDateASC(query = "",page = 1,pageSize = 100)
